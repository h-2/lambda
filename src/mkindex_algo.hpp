                   ...
                   LambdaIndexerOptions       const & options)
{
    ...
    if constexpr (std::same_as<seqan3::innermost_value_type_t<TStringSet>, seqan3::aa27>)
    {
        switch (options.indexFileOptions.redAlph)
        {
            case AlphabetEnum::MURPHY10:
            {
                auto redSeqs = seqs | seqan3::views::deep{seqan3::views::convert<seqan3::aa10murphy>};
